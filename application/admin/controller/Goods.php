...
class Goods extends BaseController
{
    ...
    public function goodseditlist()
    {
        ...
        if (request()->isAjax()) {
            ...
            $result = $goodservice->getGoodsEditList($page_index, $page_size, $condition, [
                'ng.sort' => 'asc',
                'ng.create_time' => 'desc'
            ]);
            return $result;
            ...
            $oneGoodsCategory = $goodsCategory->getGoodsCategoryListByParentId(0);
            $this->assign("oneGoodsCategory", $oneGoodsCategory);
            return view($this->style . "Goods/goodsEditList");
        }
    }
    ...
    public function addGoods()
    {
        ...
        $goods_class_list = $goods->getGoodClassList(1, 0);
        $this->assign("goods_class_list", $goods_class_list['data']); // 商品类型
       // 物流
        ...
        $express_list_pagequery = $shippingfee_list->getShippingFeeList(0, 0, ["shop_id"=>$this->instance_id], 'is_default desc,create_time desc');
        $feelist = [0=>["shipping_fee_id"=>0,"shipping_fee_name"=>"默认模板（全国包邮）"]];

        foreach ($express_list_pagequery['data'] as $value)
        {
            $feelist[] = $value;
        }
        ...
        if ($goodsId > 0) {
            ...
            if($goods_info['shop_id'] != $this->instance_id || empty($goods_info))
            {
                $this->error("当前商品信息不存在或者当前用户无权限!");
            }
            ...
            $goods_attr_id = $goods_info["goods_attribute_id"];
    ...
    public function addGoodsEdit()
    {

        $goods = new GoodsService();


        $id = isset($_GET["goodsId"]) ? $_GET["goodsId"] : 0;
        ...
        if (true) {
            ...
            $goods_info = $goods->getGoodsEditDetail($id);
    ...
    {
        $goods = new GoodsService();
        $condition["class_id"] = request()->post("attr_id", 0);
    ...
    public function deleteGoodsEdit()
    {
        $ids = request()->post('ids');
        $goodservice = new GoodsService();
        $retval = $goodservice->deleteGoodsEdit($ids);
        return AjaxReturn($retval);
    }
    ...
    public function GoodsCreateOrUpdate()
    {
        ...
        $goodservice = new GoodsService();
        $product["shopid"] = $this->instance_id;
        $res = $goodservice->addOrEditGoods($product);
    ...
    public function SubmitGoodsEdit()
    {
        $condition = $_POST["editid"]; // 将商品id用,隔开
        $goods_detail = new GoodsService();
        $result = $goods_detail->SubmitGoodsEdit($condition);
        return AjaxReturn($result);
    }

    public function UnSubmitGoodsEdit()
    {
        $condition = $_POST["editid"]; // 将商品id用,隔开
        $goods_detail = new GoodsService();
        $result = $goods_detail->UnSubmitGoodsEdit($condition);
        return AjaxReturn($result);
    }
    ...
    public function addGoodsSpec()
    {
        ...
        if (request()->isAjax()) {
            ...
            $res = $goods->addGoodsSpecService($this->instance_id, $spec_name, $show_type, $is_visible, $sort, $spec_value_str, $classid);
            return AjaxReturn($res);
        }
        $goods_attribute_list = $goods->getGoodClassList(1, 0);
        $this->assign("goods_attribute_list", $goods_attribute_list['data']); // 商品类型
    ...
    public function updateGoodsSpec()
    {
        ...
        if (request()->isAjax()) {
            ...
            $spec_value_str = request()->post('spec_value_str', '');
            $res = $goods->updateGoodsSpecService($spec_id, $this->instance_id, $spec_name, $show_type, $is_visible, $sort, $spec_value_str,$classid);
            return AjaxReturn($res);
        }
    ...
    public function updateGoodsAttribute()
    {
        ...
        if (request()->isAjax()) {
            ...
            $attr_name = isset($_POST['class_name']) ? $_POST['class_name'] : '';
            $is_use = isset($_POST['is_visible']) ? $_POST['is_visible'] : '';
            $sort = isset($_POST['sort']) ? $_POST['sort'] : '';
            $spec_id_array = isset($_POST['select_box']) ? $_POST['select_box'] : '';
            $value_string = isset($_POST['data_obj_str']) ? $_POST['data_obj_str'] : '';
            $res = $goods->updateAttributeService($class_id, $attr_name, $is_use, $spec_id_array, $sort, $value_string);
            return AjaxReturn($res);
        }
        $attribute_detail = $goods->getAttributeServiceDetail($class_id);
    ...
    {
        $goods_id = request()->post("goods_id", "");
        $url = Config::get('view_replace_str.APP_MAIN') . '/goods/goodsdetail?id=' . $goods_id;

        try {
            $pay_qrcode = getQRcode($url, 'upload/goods_qrcode', 'goods_qrcode_' . $goods_id);
        } catch (\Exception $e) {
            return AjaxReturn(UPLOAD_FILE_ERROR);
        }
        $goods = new GoodsService();
        $result = $goods->goods_QRcode_make($goods_id, $pay_qrcode);
        return AjaxReturn($result);
    }
}