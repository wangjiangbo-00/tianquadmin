...
class Expressnotify extends Controller
{
    ...
    public function on56NotifyMessage()
    {
        ...
        trace("receive from 56 notify with message");
        if(isset($_POST['trail']))
        {
            $trail = json_decode($_POST['trail'], true);
            if(isset($trail['information']))
            {
                $info = $trail['information'];
                $exp = $info['exp'];
                $num = $info['num'];
                $data = $info['data'];
                ...
                $order_express = $order_express_model->getInfo([
                    'company_code' => $exp,
                    'express_no' => $num,
                ]);

                $order_express['message'] = $data;

                $order_express_model->save($order_express);
                ...
                trace("receive from 56 notify with message".$num.$exp.$data);
