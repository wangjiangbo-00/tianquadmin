...
class Config extends BaseService implements IConfig
{
    ...
    public function getGiftFreeChance($shop_id){
        $register_and_visit = $this->getConfig($shop_id, 'GIFTFREECHANCE');
        if(empty($register_and_visit))
        {
            //按照默认值显示生成
    ...
    public function setGiftFreeChance($shop_id, $is_open, $chance){
        ...
        $config_model=new ConfigModel();
        $res = $config_model->save($data,['key'=>'GIFTFREECHANCE','instance_id'=>$shop_id]);
        return $res;
    }
