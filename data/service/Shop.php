    ...
    */
    ...
    public function getShopDetail($shop_id)
    {
        ...
        if (! empty($base_info)) {
            ...
            {
                $shop_info['base_info']['shop_state_str'] = "开启";
            }
            else
            {
                $shop_info['base_info']['shop_state_str'] = "关闭";
            }
        }
        return $shop_info;
    }
    ...
    public function addshop($shop_name, $shop_type, $uid, $shop_group_id, $shop_company_name, $province_id, $city_id, $shop_address, $shop_zip, $shop_sort, $recommend_uid = 0)
    {
        ...
        $province = $provincemodel->getInfo(['province_id'=>$province_id]);
        $citymodel = new CityModel();
        $city = $citymodel->getInfo(['city_id'=>$city_id]);
        // 防止出现重复店铺、重复提交问题
        if ($count > 0) {
            return - 1;
        }
    ...
    public function GetShopBannerDetails($id)
    {

        $shopbanner = new NsPlatformBannerModel();
        $info = $shopbanner->get($id);
        return $info;
    }
    public function addShopBanner($name, $type,$refid,$refdetails, $pic, $is_visible,$is_valid,$description,$sort)
    {
        $shopBanner = new NsPlatformBannerModel();
        $shopBanner->startTrans();
        try {
    ...
    public function updateShopBanner($id, $description,$pic,$is_visible,$is_valid,$sort)
    {
        $shopBanner = new NsPlatformBannerModel();
        $shopBanner->startTrans();
        try {
...
//             $instance_arr = rtrim($instance_arr, ",");
    ...
    public function updateShopConfigByshop($shop_id, $shop_logo, $shop_banner, $shop_avatar, $shop_qrcode, $shop_qq, $shop_ww, $shop_phone, $shop_keywords, $shop_description)
    {
        ...
        $redis = new Redis();
        $redis->set("apicheck.getrecommendshops",$shop_id);
        return $res;
    }
